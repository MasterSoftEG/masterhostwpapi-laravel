...
{
    $this->apiKey = $apiKey ?? Config::get('masterhosteg.api_key', env('masterhosteg_API_KEY'));
    $this->baseUrl = rtrim(Config::get('masterhosteg.base_url', env('masterhosteg_BASE_URL', 'https://masterhosteg.com/api')), '/');
    $this->personalAccessToken = Config::get('masterhosteg.personal_access_token', env('masterhosteg_PERSONAL_ACCESS_TOKEN'));
}
    /**
     * Send a text message.
     *
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendImageMessageData) {
            $payload = [
                'to' => $to->to,
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendVideoMessageData) {
            $payload = [
                'to' => $to->to,
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendDocumentMessageData) {
            $payload = [
                'to' => $to->to,
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendAudioMessageData) {
            $payload = [
                'to' => $to->to,
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendStickerMessageData) {
            $payload = [
                'to' => $to->to,
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendContactMessageData) {
            $payload = [
                'to' => $to->to,
                'contact' => [
                    'name' => $to->contactName,
    ...
    {
        if ($to instanceof \masterhosteg\Data\SendLocationMessageData) {
            $location = [
    ...
    protected function request(string $method, string $path, ?array $payload = null, bool $usePersonalToken = false): array
    {
        if ($usePersonalToken && empty($this->personalAccessToken)) {
            throw new masterhostegException('this endpoint requires a personal access token');
        }
        $url = $this->baseUrl . $path;
        $headers = [
            'Accept' => 'application/json',
            'Authorization' => 'Bearer ' . ($usePersonalToken && $this->personalAccessToken ? $this->personalAccessToken : $this->apiKey),
            'User-Agent' => 'masterhosteg-laravel-sdk',
        ];
        ...
        if (!$resp->successful()) {
            throw new masterhostegException('masterhost API error: ' . $resp->body(), $resp->status(), $resp->json());
        }
        return $resp->json();
    }
    ...
    protected function postWithRetry(string $path, array $payload, bool $usePersonalToken = false, ?RetryConfig $retryConfig = null): array
    {
        ...
        throw new masterhostegException('Max retries exceeded', 429);
    }
} 
